...
FROM --platform=$BUILDPLATFORM golang:1.23 AS zigdownloader-amd64
...
FROM --platform=$BUILDPLATFORM golang:1.23 AS zigdownloader-arm64
...
FROM --platform=$BUILDPLATFORM golang:1.23 AS gobase-amd64

ENV ZIGTARGET x86_64-linux-gnu

# Prepare for targetarch arm64
FROM --platform=$BUILDPLATFORM golang:1.23 AS gobase-arm64
