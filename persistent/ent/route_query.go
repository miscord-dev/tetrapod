...
func (rq *RouteQuery) QueryHost() *NodeQuery {
	query := &NodeQuery{config: rq.config}
...
func (rq *RouteQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (rq *RouteQuery) FirstIDX(ctx context.Context) int64 {
...
func (rq *RouteQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (rq *RouteQuery) OnlyIDX(ctx context.Context) int64 {
...
func (rq *RouteQuery) IDs(ctx context.Context) ([]int64, error) {
	var ids []int64
...
func (rq *RouteQuery) IDsX(ctx context.Context) []int64 {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *RouteQuery) WithHost(opts ...func(*NodeQuery)) *RouteQuery {
	query := &NodeQuery{config: rq.config}
...
func (rq *RouteQuery) sqlAll(ctx context.Context) ([]*Route, error) {
	...
	if query := rq.withHost; query != nil {
		ids := make([]int64, 0, len(nodes))
		nodeids := make(map[int64][]*Route)
		for i := range nodes {
			if nodes[i].node_routes == nil {
				continue
			}
			fk := *nodes[i].node_routes
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "node_routes" returned %v`, n.ID)
			}
			for i := range nodes {
