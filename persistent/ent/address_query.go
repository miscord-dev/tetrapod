...
func (aq *AddressQuery) QueryHost() *NodeQuery {
...
func (aq *AddressQuery) WithHost(opts ...func(*NodeQuery)) *AddressQuery {
	query := &NodeQuery{config: aq.config}
...
func (aq *AddressQuery) sqlAll(ctx context.Context) ([]*Address, error) {
	...
	if query := aq.withHost; query != nil {
		...
		for i := range nodes {
			if nodes[i].node_addresses == nil {
				continue
			}
			fk := *nodes[i].node_addresses
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "node_addresses" returned %v`, n.ID)
			}
			for i := range nodes {
